...
class PerfFetcher(PerformanceMixin, BaseFetcher):
    pass


@pytest.fixture
def fetcher():
    return PerfFetcher(YaMetrikaClient(YaMetrikaConfig(api_key="tok")))


@pytest.mark.asyncio
async def test_get_page_performance(httpx_mock, fetcher):
    httpx_mock.add_response(url=re.compile(r".*stat/v1/data.*"), json={"data": []})
    result = await fetcher.get_page_performance("12345")
